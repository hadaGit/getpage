...
import Plug, { PlugConfig } from './plugload/Plug';
import { app } from 'electron';

const plugsPath = path.resolve(app.getPath('userData'), 'plugs');
...
export function setConfig(name: string, config: PlugConfig) {
  const plug = Plug.getPlugByName(name);
  return plug.setConfig(config);
}
